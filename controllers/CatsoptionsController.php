<?php
	...
	class CatsoptionsController extends Controller {
		...
		public function actionDeleteOption() {
			...
			$model_name = 'Cats';
			$option     = ModelsOptionsList::find()->where(['model_id'   => $model_id,
			                                                'model_name' => $model_name,
			                                                'option_id'  => $option_id
			])->one()
			;
		...
		protected function getAllParentOptions($model_name, $model_id) {
			...
			/** @var $mn ActiveRecord */
			/** @var $cat Cats */
			$mn  = MyHelper::ADMIN_MODEL_NAMESPACE . $model_name;
			$cat = $mn::findOne(['id' => $model_id]);
			if($cat->parent->id != 0) {
				$options_arr = ArrayHelper::merge($this->getChildOptions($model_name, $cat->parent->id), $this->getAllParentOptions($model_name, $cat->parent->id));
			} /*else {
